...
export function PurchasesPage({ user, stores, selectedStore }: Props) {
  ...
  const resource = usePagedList<PurchaseOrder>(queryPath('/purchase-orders', { store_id: storeParam(selectedStore), q: query, status, payment_status: paymentStatus, invoice_status: invoiceStatus }));
  ...
  return <>
    ...
    <Card className="section-card" title="采购单" extra={<Button icon={<ReloadOutlined />} onClick={resource.reload}>刷新</Button>}>
      ...
      <Table<PurchaseOrder> rowKey="id" dataSource={resource.data?.items ?? []} loading={resource.loading} pagination={resource.pagination} scroll={{ x: 1100 }} locale={{ emptyText: <EmptyState text="暂无采购记录。先维护商品和供应商，再建立采购单。" /> }} columns={[
        ...
        { title: '付款 / 发票', width: 155, render: (_, item) => <>{paymentStatuses[item.payment_status]}<small className="cell-secondary">{invoiceStatuses[item.invoice_status]}</small></> },
...
function PurchaseEditor({ user, order, stores, selectedStore, onClose, onSaved }: { user: User; order: PurchaseOrder | null; stores: Store[]; selectedStore: string; onClose: () => void; onSaved: () => void }) {
  ...
  const productPath = storeId && supplierId ? queryPath('/products', { is_active: true, store_id: storeId, supplier_id: supplierId }) : null;
  ...
  return <Modal open title={order ? '编辑采购草稿' : '新增采购单'} width={1000} onCancel={saving ? undefined : onClose} closable={!saving} mask={{ closable: false }} onOk={() => form.submit()} confirmLoading={saving} okText={order ? "保存草稿" : "保存采购单"}>
    ...
    <Form form={form} layout="vertical" onFinish={save} onValuesChange={changes => { if (Object.hasOwn(changes, 'store_id') || Object.hasOwn(changes, 'supplier_id')) { form.setFieldValue('lines', [{ quantity: 1, unit_price: '0' }]); setEditingProduct(undefined); } }} initialValues={order ? { ...order, expected_date: order.expected_date || '', planned_ship_date: order.planned_ship_date || '', lines: order.lines } : { store_id: storeParam(selectedStore), order_date: dayjs().format('YYYY-MM-DD'), currency: 'CNY', lines: [{ quantity: 1, unit_price: '0' }] }}>
