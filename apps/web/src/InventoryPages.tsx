...
import { movementKinds, options, RemoteSelect, requestId, required, StoreField, storeParam, warehouseKinds } from './SupplyShared';
...
export function InventoryPage({ user, stores, selectedStore }: { user: User; stores: Store[]; selectedStore: string }) {
  const [q, setQ] = useState(''); const query = useDebouncedValue(q); const [warehouseKind, setWarehouseKind] = useState('fba'); const [editing, setEditing] = useState<InventoryBalance | null | undefined>();
  const [tab, setTab] = useState('balances'); const [version, setVersion] = useState(0);
  const scope = { store_id: storeParam(selectedStore), warehouse_kind: warehouseKind === 'all' ? undefined : warehouseKind };
  ...
  return <>
    <PageHeading eyebrow="INVENTORY CONTROL" title="库存管理" description="默认按 FBA 仓库核对各店铺商品的实物、占用、可用与在途数量，每次变动保留单据流水。" extra={user.permissions.includes('inventory.adjust') && <Button type="primary" icon={<PlusOutlined />} onClick={() => setEditing(null)}>期初 / 库存调整</Button>} />
    ...
    <Alert className="page-notice" type="info" showIcon title="数量口径" description="摘要与余额、流水采用相同仓库范围，默认查看 FBA 仓库；在途仅计算已发出未接收数量。FBA 仓为人工接收的账面记录，实时可售库存需后续导入亚马逊快照核对。" />
    <div className="catalog-filter-bar"><Select aria-label="库存仓库范围" style={{ width: 240 }} value={warehouseKind} onChange={setWarehouseKind} options={[{ value: 'fba', label: 'FBA仓库' }, { value: 'all', label: '全部仓库（含历史记录）' }]} /></div>
    <Tabs activeKey={tab} onChange={setTab} items={[
      ...
      { key: 'movements', label: '库存流水', children: tab === 'movements' ? <MovementList key={version} selectedStore={selectedStore} warehouseKind={warehouseKind} /> : null },
...
function MovementList({ selectedStore, warehouseKind }: { selectedStore: string; warehouseKind: string }) {
  const [kind, setKind] = useState<string>();
  const resource = usePagedList<Movement>(queryPath('/inventory/movements', { store_id: storeParam(selectedStore), kind, warehouse_kind: warehouseKind === 'all' ? undefined : warehouseKind }));
...
function WarehouseList({ user }: { user: User }) {
  ...
  return <><ErrorNotice error={resource.error} retry={resource.reload} /><Card className="section-card" title="公司共享仓库档案" extra={canManage && <Button icon={<PlusOutlined />} onClick={() => setEditing(null)}>新增仓库</Button>}>
    ...
    ]} locale={{ emptyText: <EmptyState text="默认使用 FBA仓库，首次发货或登记库存时自动建立，无需维护具体仓点。" /> }} />
...
function WarehouseEditor({ warehouse, onClose, onSaved }: { warehouse: Warehouse | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal open title={warehouse ? '编辑仓库' : '新增仓库'} onCancel={saving ? undefined : onClose} closable={!saving} mask={{ closable: false }} onOk={() => form.submit()} confirmLoading={saving}>
    <ErrorNotice error={error} /><Form form={form} layout="vertical" initialValues={warehouse || { kind: 'fba', name: 'FBA仓库', is_active: true }} onFinish={async values => {
...
function AdjustmentEditor({ balance, stores, selectedStore, onClose, onSaved }: { balance: InventoryBalance | null; stores: Store[]; selectedStore: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal open title="期初 / 库存调整" onCancel={saving ? undefined : onClose} closable={!saving} mask={{ closable: false }} onOk={() => form.submit()} confirmLoading={saving} okText="确认过账">
    ...
    }}><StoreField stores={stores} fixed={!!balance} /><Form.Item name="warehouse_id" hidden><Input /></Form.Item><Form.Item label="仓库"><Input value={balance?.warehouse_name || 'FBA仓库'} readOnly /></Form.Item><Form.Item name="product_id" label="商品 / SKU" rules={required}><RemoteSelect path="/products?is_active=true" disabled={!!balance} initialLabel={balance ? `${balance.internal_sku} · ${balance.product_name}` : undefined} /></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="kind" label="类型" rules={required}><Select options={[{ value: 'opening', label: '期初库存' }, { value: 'adjustment', label: '库存调整' }]} /></Form.Item></Col><Col span={12}><Form.Item name="quantity" label="变动数量" rules={[...required, { validator: async (_, value) => { if (!value) throw new Error('数量不能为零'); } }]}><InputNumber precision={0} min={-1000000000} max={1000000000} style={{ width: '100%' }} /></Form.Item></Col></Row><Form.Item name="reason" label="原因 / 盘点依据" rules={required}><Input.TextArea rows={3} maxLength={2000} /></Form.Item></Form>
  </Modal>;
}
