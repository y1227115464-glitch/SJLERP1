...
from app.models import Supplier, User, new_id
...
from app.supply.schemas import PurchaseInput
...
def apply_purchase(db, record, payload, user):
    ...
    for key, value in payload.model_dump(exclude={'request_id', 'lines'}).items():
...
def create(payload: PurchaseInput, db: DB, user: Writer):
    active_store(db, user, payload.store_id)
...
def confirm(identifier: str, db: DB, user: Writer):
    ...
    audit(db, user, 'purchases.confirm', 'purchase_order', identifier, '提交采购单', record.store_id)
    db.commit()
    return purchase_out(record, user)
